...
multiset<PII> :: reverse_iterator it;
...
PII getV(int x, multiset<PII> &s, int expect = -1) {
	int self = -inf;
	if (color[x] == WHITE)
		self = 0;
	while(!s.empty()) {
...
void doit(int x) {
	...
	while(x) {
		...
		if (x) {
			heap[x].insert(make_pair(g[s] + fv[s], s));
		}
		f[s] = tmp.opt;
